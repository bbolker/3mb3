...
Basic model: $\bm x(t+1) = \bm A \bm x(t)$. 
...
Four approaches:
\begin{description}
\item[recursion] $\bm x(1) = \bm A \bm x(0)$, then $\bm x(2) =
...
(in Python: \texttt{numpy.linalg.matrix\_power}), so that $\bm x(t) =
    \bm A^t \bm x(0)$. This is efficient but doesn't provide much
    insight. 
  \item[diagonalization] Gain insight by \emph{diagonalizing} $\bm A =
   ...
   \item[series] Let $\bm c = \bm S^{-1} \bm x(0)$. This
    ...
    \qq{What happens when $d_1 = d_2$? Try it out in Python}.
...
If $\bm A = \begin{bmatrix} a_{11} & a_{12} \\ a_{21} &
  ...
  eigenvalues obey $d_i = (T \pm \sqrt{T^2 -
  ...
  Example: for the juvenile-adult model, we have $d_i = (s_A
